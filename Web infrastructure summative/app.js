...
class EventFinder {
    ...
    async handleSearch() {
        ...
        if (!this.apiKey) {
            this.showError('API key is not configured. Please check your config.js file.');
            return;
        }
    ...
    async fetchEvents(city) {
        ...
        const apiUrl = `https://serpapi.com/search.json?engine=google_events&q=${encodeURIComponent(query)}&api_key=${this.apiKey}`;
